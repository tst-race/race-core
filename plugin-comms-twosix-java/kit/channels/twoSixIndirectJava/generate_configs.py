...
def main() -> None:
    """
    Purpose:
        Generate configs for twoSixIndirectJava
    Args:
        N/A
    Returns:
        N/A
...
def generate_genesis_link_addresses(
    ...
    base_client_hashtag = "java_clients_{link_idx}"
    base_server_hashtag = "java_servers_{link_idx}"
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        stream=sys.stdout,
        level=LOG_LEVEL,
        format="[generate_comms_java_indirect_configs] %(asctime)s.%(msecs)03d %(levelname)s %(message)s",
