...
def main() -> None:
    """
    Purpose:
        Generate configs for twoSixIndirectRust
    Args:
        N/A
    Returns:
        N/A
...
def generate_genesis_link_addresses(
    ...
    base_client_hashtag = "rust_clients_{link_idx}"
    base_server_hashtag = "rust_servers_{link_idx}"
        ...
        link_addresses[sender_node].append(
            {
                ...
                "address": f'{{"port" : {two_six_whiteboard_details.get("""port""", None)}, "hostname" : "{two_six_whiteboard_details.get("""hostname""", None)}", "hashtag": "{hashtag}", "checkFrequency": 1000}}',
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        stream=sys.stdout,
        level=LOG_LEVEL,
        format="[generate_comms_rust_indirect_configs] %(asctime)s.%(msecs)03d %(levelname)s %(message)s",
