...
bool channelLinksFull(IRaceSdkNM *sdk, const std::string &channelGid) {
    logDebug("Checking if channel has reached max links...");
    ChannelProperties properties = sdk->getChannelProperties(channelGid);
    std::vector<LinkID> links = sdk->getLinksForChannel(channelGid);
    int numMaxLinks = properties.maxLinks;
    int numLinks = static_cast<int>(links.size());
    logDebug("Number of links on channel " + channelGid + ": " + std::to_string(numLinks));
    logDebug("Number of max links for channel " + channelGid + ": " + std::to_string(numMaxLinks));
    return (numLinks >= numMaxLinks);
}
