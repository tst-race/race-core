...
CHANNEL_ID = "twoSixIndirectPython"
try:
    with open(
...
def main() -> None:
    """
    Purpose:
        Generate configs for twoSixIndirectPython
    Args:
        N/A
    Returns:
        N/A
        ...
        link_addresses[sender_node].append(
            {
                ...
                "address": f'{{"port" : {two_six_whiteboard_details.get("""port""", None)}, "hostname" : "{two_six_whiteboard_details.get("""hostname""", None)}", "hashtag": "{hashtag}", "checkFrequency": 1000, "multicast": "True"}}',
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        stream=sys.stdout,
        level=LOG_LEVEL,
        format="[generate_comms_python_indirect_configs] %(asctime)s.%(msecs)03d %(levelname)s %(message)s",
