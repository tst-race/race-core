 ...
 */
template <typename T>
bool saveValue(IRaceSdkCommon *sdk, const std::string &key, T value);
 ...
 */
template <typename T>
T readValue(IRaceSdkCommon *sdk, const std::string &key, T defaultValue);

}  // namespace psh

template <typename T>
bool psh::saveValue(IRaceSdkCommon *sdk, const std::string &key, T value) {
    std::string valueString = std::to_string(value);
    return sdk->writeFile(key, {valueString.begin(), valueString.end()}).status == SDK_OK;
}

template <typename T>
T psh::readValue(IRaceSdkCommon *sdk, const std::string &key, T default_value) {
