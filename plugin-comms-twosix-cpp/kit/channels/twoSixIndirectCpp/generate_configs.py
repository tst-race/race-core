...
def main() -> None:
    ...
    additional_attributes = {
        ...
        "max_poll_retries": cli_args.max_poll_retries,
    }
...
def generate_genesis_link_addresses(
    ...
    maxTries = additional_attributes["max_poll_retries"]
        ...
        link_addresses[sender_node].append(
            {
                ...
                "address": f'{{"port" : {two_six_whiteboard_details.get("""port""", None)}, "hostname" : "{two_six_whiteboard_details.get("""hostname""", None)}", "hashtag": "{hashtag}", "maxTries": {maxTries}}}',
...
def get_cli_arguments() -> argparse.Namespace:
    ...
    optional.add_argument(
        "--max-poll-retries",
        dest="max_poll_retries",
        help="The number of times a poll request should be re-tried before it is deemed unavailable.",
        required=False,
        default=600,
        type=int,
    )
