...
# Start and end ports for c++ exemplar port range.
START_PORT = 20_000
END_PORT = 21_999
...
def generate_user_responses(
    ...
    # This is identical to the code for bootstrap links. Both write to the same entries.
    # When Rib merges the user response it will only take one, but both are necessary
    # in case we're e.g. using bootstrap links with other performer exemplars.
    try:
...
def get_cli_arguments() -> argparse.Namespace:
    ...
    optional.add_argument(
        ...
        help="Proportion of messages corrupted when sending for testing purposes",
        required=False,
        default=0,
        type=float,
    )
    optional.add_argument(
        ...
        help="Proportion of messages corrupted when receiving for testing purposes",
        required=False,
        default=0,
        type=float,
    )
    optional.add_argument(
        ...
        help="If a message was selected for corruption, how much should it be corrupted, in bytes",
        required=False,
        default=0,
        type=float,
    )
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        stream=sys.stdout,
        level=LOG_LEVEL,
        format="[generate_comms_cpp_direct_configs] %(asctime)s.%(msecs)03d %(levelname)s %(message)s",
